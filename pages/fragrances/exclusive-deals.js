...
const exclusiveFrag = ({products}) => {
  return (
    ...
    <Main>
          ...
          <Title>EXCLUSIVE DEALS ON FRAGRANCES</Title>
          <ImageContainer>
          <Image src={InvertedLogo} alt="Vallonie Logo" />
          </ImageContainer>
      </Main>
        <Divider>
            <DividerText>
            LIMITED OFFERS ! ------------------------ EXCLUSIVE DEALS  ! 
                ------------------------ GOLDEN OPPURTINTIY ! 
            </DividerText>
        </Divider>
        <Subtext>BROWSE THE WORLD CLASS TOP NODGE COLOGNES. ESSENSE OF EUROPEAN CRAFT, DESIGN, AND ART COMBINED TO PREFECTION IN YOUR HAND.</Subtext>
...
export const getServerSideProps = async () => {
    const query1 = '*[_type == "allMenFragProduct"]';
    const query2 = '*[_type == "allWomenFragProduct"]';
    const query3 = '*[_type == "unisexFragProduct"]';
    ...
    for (let i = 0; i < products1.length; i++){
      if (products1[i].discountOn){
        products.push(products1[i]);
      }
    }
    for (let i = 0; i < products2.length; i++){
      if (products2[i].discountOn){
        products.push(products2[i]);
      }
    }
    for (let i = 0; i < products3.length; i++){
      if (products3[i].discountOn){
        products.push(products3[i]);
      }
    }
